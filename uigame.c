...
--*/
{
	...
	//	{  0,      MAINMENU_LABEL_NEWGAME,   TRUE,     PAL_XY(128 - (w[0] > 4 ? (w[0] - 4) * 8 : 0), 139) },
	//	{  1,      MAINMENU_LABEL_LOADGAME,  TRUE,     PAL_XY(128 - (w[1] > 4 ? (w[1] - 4) * 8 : 0), 156) },
	//	{  MENUITEM_VALUE_CANCELLED - 1,      SYSMENU_LABEL_QUIT,  TRUE,     PAL_XY(128 - (w[2] > 4 ? (w[2] - 4) * 8 : 0), 173) }
	//};

	MENUITEM      rgMainMenuItem[3] = {
		// value   label                     enabled   position
		{  0,      MAINMENU_LABEL_NEWGAME,   TRUE,     PAL_XY((320 - wcslen(PAL_GetWord(MAINMENU_LABEL_NEWGAME)) * 16) / 2, 139) },
		{  1,      MAINMENU_LABEL_LOADGAME,  TRUE,     PAL_XY((320 - wcslen(PAL_GetWord(MAINMENU_LABEL_LOADGAME)) * 16) / 2, 156) },
		{  2,      SYSMENU_LABEL_QUIT,  TRUE,     PAL_XY((320 - wcslen(PAL_GetWord(SYSMENU_LABEL_QUIT)) * 16) / 2, 173) }
	};
	...
	while (TRUE)
	{
		...
		// Activate the menu
		// 列出菜单项
		wItemSelected = PAL_ReadMenu(NULL, rgMainMenuItem, 3, wDefaultItem, MENUITEM_COLOR);
		...
		if (wItemSelected == 0)
		{
			...
			{
				wItemSelected = 0;
				gpGlobals->wGameDifficulty = wDifficultyNum;
				break;
			}
		}
		...
		else if (wItemSelected == MENUITEM_VALUE_CANCELLED) {
			wDefaultItem = 0;
		}
	}
...
--*/
{
	...
	MENUITEM        rgMenuItem[SAVE_NUMBEROFAUTO];

	const SDL_Rect  rect = { 195 - dx, 7, 120 + dx, 190 };

	//
	// Create the boxes and create the menu items，2022年12月13日19点25分修改，存档个数修改（存盘号）
	//
	lpSaveMenuBox = PAL_CreateBox(PAL_XY(200 - dx, 12), 8, 4, 1, FALSE);

	PAL_DrawText(L"❤进度回溯❤", PAL_XY(206, 1), 61, TRUE, FALSE, FALSE);

	for (i = 0; i < SAVE_NUMBEROFAUTO; i++)
	{
		...
		if (i >= 5)
			rgMenuItem[i].wNumWord = LOADMENU_LABEL_SLOT_SIXTH + i - 5;
		else
	...
	//
	// Draw the numbers of saved times，2022年12月13日19点25分修改，存档个数修改（存盘次数）
	//
	// 便捷存档应当以青色数字显示
	PAL_DrawNumber((UINT)GetSavedTimes(9), 5, PAL_XY(270, 160),
		kNumColorCyan, kNumAlignRight);

	// 自动存档应当以蓝色数字显示
	PAL_DrawNumber((UINT)GetSavedTimes(10), 5, PAL_XY(270, 177),
		kNumColorBlue, kNumAlignRight);

	// 其他手动存档应当以白色数字显示
	for (i = 1; i <= MAX_SAVE_NUMBER; i++)
	{
		//
		// Draw the number
		//
		...
		PAL_DrawNumber((UINT)GetSavedTimes(i), 5, PAL_XY(270, 7 + 17 * i),
			kNumColorYellow, kNumAlignRight);
	}
	...
	// Activate the menu
	//
	wItemSelected = PAL_ReadMenu(NULL, rgMenuItem, SAVE_NUMBEROFAUTO, wDefaultSlot - 1, MENUITEM_COLOR);

	//
	// Delete the boxes
	// 清除该框
	PAL_DeleteBox(lpSaveMenuBox);
...
--*/
{
	WORD wItems[2] = { CONFIRMMENU_LABEL_YES, CONFIRMMENU_LABEL_NO };
	WORD wReturnValue = PAL_SelectionMenu(2, 0, wItems);

	return (wReturnValue == MENUITEM_VALUE_CANCELLED || wReturnValue == 1) ? FALSE : TRUE;
...
--*/
{
	WORD wItems[4] = { CONFIRMMENU_LABEL_YES, CONFIRMMENU_LABEL_NO, CONFIRMMENU_LABEL_FIVE ,CONFIRMMENU_LABEL_TEN };
...
--*/
{
	...
	// 绘制金钱数目
	PAL_DrawNumber(dwCash, 6, PAL_XY(49, 14), kNumColorGold, kNumAlignRight);
...
--*/
{
	LPBOX               lpMenuBox;
	WORD                wReturnValue;
	int                 iSlot = MENUITEM_VALUE_CANCELLED, i;
	...
	case 1:
		...
		if (iSlot != MENUITEM_VALUE_CANCELLED)
		{
			...
			gpGlobals->fIsArchiveCompleted = TRUE;

			for (i = 1; i <= SAVE_NUMBEROFAUTO; i++)
				wSavedTimes = (GetSavedTimes(i) > wSavedTimes) ? GetSavedTimes(i) : wSavedTimes;
...
--*/
{
  ...
  // 2022年12月13日13点59分修改，非战斗时显示HPMP信息框
		PAL_PlayerInfoBox(PAL_XY(77 * i + 7, 165), gpGlobals->rgParty[i].wPlayerRole,
	...
	while (TRUE)
	{
		...
		if (gpGlobals->g.rgObject[wMagic].magic.wFlags & kMagicFlagApplyToAll)
		{
			gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse =
				PAL_RunTriggerScript(gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse, 0, FALSE);

			if (g_fScriptSuccess)
			{
				gpGlobals->g.rgObject[wMagic].magic.wScriptOnSuccess =
					PAL_RunTriggerScript(gpGlobals->g.rgObject[wMagic].magic.wScriptOnSuccess, 0, FALSE);
	 ...
	 // 2022年12月13日14点48分修改，
					PAL_PlayerInfoBox(PAL_XY(77 * i + 7, 165), gpGlobals->rgParty[i].wPlayerRole, 100,
						TIMEMETER_COLOR_DEFAULT, TRUE);
					y += 78;
				}
					...
					else if (g_InputState.dwKeyPress & kKeySearch)
					{
						gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse =
							PAL_RunTriggerScript(gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse,
								gpGlobals->rgParty[wPlayer].wPlayerRole, FALSE);
...
out:
	...
	// 若玩家存档，则保存游戏截图
	if (gpGlobals->fIsArchiveCompleted)
	{
		gpGlobals->fIsArchiveCompleted = FALSE;
		VIDEO_SaveScreenshot(gpGlobals->bCurrentSaveSlot);
	}
}
...
--*/
{
	...
	int              labels0[] = {
	   ...
	   STATUS_LABEL_MP, STATUS_LABEL_SP
	};
	int              labels1[] = {
	   STATUS_LABEL_EXP_LAYOUT, STATUS_LABEL_LEVEL_LAYOUT, STATUS_LABEL_HP_LAYOUT,
	   STATUS_LABEL_MP_LAYOUT, STATUS_LABEL_SP_LAYOUT
	};
	int              labels[] = {
	...
	{
		wPlayerRole = gpGlobals->rgParty[iCurrent].wPlayerRole;
		...
		// 绘制队员角色的肖像
		if (PAL_MKFReadChunk(bufImage, PAL_RLEBUFSIZE, gpGlobals->g.PlayerRoles.rgwAvatar[wPlayerRole], gpGlobals->f.fpRGM) > 0)
		{
		...
		for (i = 0; i < MAX_PLAYER_EQUIPMENTS; i++)
		{
			...
			w = gpGlobals->g.PlayerRoles.rgwEquipment[i][wPlayerRole];
			...
			// Draw the image
			// 绘制装备图像
			if (PAL_MKFReadChunk(bufImage, PAL_RLEBUFSIZE, gpGlobals->g.rgObject[w].item.wBitmap, gpGlobals->f.fpBALL) > 0)
			{
			...
			PAL_DrawText(PAL_GetWord(w), PAL_XY(PAL_X(gConfig.ScreenLayout.RoleEquipImageBoxes[i]) + 25 - offset / 2, PAL_Y(gConfig.ScreenLayout.RoleEquipNames[i])), STATUS_COLOR_EQUIPMENT, fShadow, FALSE, fUse8x8Font);
		}
		...
		// 绘制队员名称文本标签
		PAL_DrawText(PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[wPlayerRole]), gConfig.ScreenLayout.RoleName, MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		...
		PAL_DrawNumber(gpGlobals->Exp.rgPrimaryExp[wPlayerRole].wExp, 7,
			gConfig.ScreenLayout.RoleCurrExp, kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[wPlayerRole]], 7,
			gConfig.ScreenLayout.RoleNextExp, kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwLevel[wPlayerRole], 4,
			gConfig.ScreenLayout.RoleLevel, kNumColorYellow, kNumAlignRight);

		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleCurHP, kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleMaxHP, kNumColorBlue, kNumAlignRight);

		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleCurMP, kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleMaxMP, kNumColorBlue, kNumAlignRight);

		// 绘制精力值
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwSP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleCurSP, kNumColorGold, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxSP[wPlayerRole], 6,
			gConfig.ScreenLayout.RoleMaxSP, kNumColorBlue, kNumAlignRight);

		PAL_DrawNumber(PAL_GetPlayerAttackStrength(wPlayerRole), 5,
			gConfig.ScreenLayout.RoleStatusValues[0], kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerMagicStrength(wPlayerRole), 5,
			gConfig.ScreenLayout.RoleStatusValues[1], kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDefense(wPlayerRole), 5,
			gConfig.ScreenLayout.RoleStatusValues[2], kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDexterity(wPlayerRole), 5,
			gConfig.ScreenLayout.RoleStatusValues[3], kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerFleeRate(wPlayerRole), 5,
			gConfig.ScreenLayout.RoleStatusValues[4], kNumColorYellow, kNumAlignRight);

		// 绘制我方灵抗
		PAL_DrawText(L"灵抗", gConfig.ScreenLayout.SpiritsResistance, MENUITEM_COLOR, TRUE, FALSE, FALSE);

		for (i = 0; i < NUM_MAGIC_ELEMENTAL; i++)
		{
			w = PAL_GetPlayerElementalResistance(wPlayerRole, i);
			PAL_DrawNumber(min(100, w), 3, gConfig.ScreenLayout.SpiritsResistanceValues[i], magicsResistanceColor[i], kNumAlignRight);
		}

		// 绘制我方毒抗
		PAL_DrawText(L"毒", gConfig.ScreenLayout.OtherResistance[0], MENUITEM_COLOR, TRUE, FALSE, FALSE);
		w = PAL_GetPlayerPoisonResistance(wPlayerRole);
		PAL_DrawNumber(min(100, w), 3, gConfig.ScreenLayout.OtherResistanceValues[0], kNumColorCyan, kNumAlignRight);

		// 绘制我方物抗┭┮﹏┭┮
		PAL_DrawText(L"物", gConfig.ScreenLayout.OtherResistance[1], MENUITEM_COLOR, TRUE, FALSE, FALSE);
		w = PAL_New_GetPlayerPhysicalResistance(wPlayerRole);
		PAL_DrawNumber(min(100, w), 3, gConfig.ScreenLayout.OtherResistanceValues[1], kNumColorGold, kNumAlignRight);

		// 绘制我方巫抗
		PAL_DrawText(L"巫", gConfig.ScreenLayout.OtherResistance[2], MENUITEM_COLOR, TRUE, FALSE, FALSE);
		w = PAL_New_GetPlayerSorceryResistance(wPlayerRole);
		PAL_DrawNumber(min(100, w), 3, gConfig.ScreenLayout.OtherResistanceValues[2], kNumColorPink, kNumAlignRight);

		// 绘制我方绝抗
		PAL_DrawText(L"绝", gConfig.ScreenLayout.OtherResistance[3], MENUITEM_COLOR, TRUE, FALSE, FALSE);
		w = PAL_New_GetPlayerUniqueSkillResistance(wPlayerRole);
		PAL_DrawNumber(min(100, w), 3, gConfig.ScreenLayout.OtherResistanceValues[3], kNumColorBlue, kNumAlignRight);
...
--*/
{
	...
	SDL_Rect       rect = { 121, 2, 310, 200 };
	int            i, j, iItemBackgroundPosX, iItemBackgroundPosY;

	bSelectedColor = MENUITEM_COLOR_SELECTED_FIRST;
	dwColorChangeTime = 0;
	LPCWSTR lpItemPicText;
	...
	while (TRUE)
	{
		...
		// 绘制队员属性名称
		j = 0;
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_LEVEL), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_HP), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_MP), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_SP), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_ATTACKPOWER), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_MAGICPOWER), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_RESISTANCE), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_DEXTERITY), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_FLEERATE), PAL_XY(201, 16 + 18 * j++),
			ITEMUSEMENU_COLOR_STATLABEL, TRUE, FALSE, FALSE);

		i = gpGlobals->rgParty[wSelectedPlayer].wPlayerRole;

		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwLevel[i], 5, PAL_XY(260, 20),
		...
		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
			PAL_XY(268, 38 + 18 * j));
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[i], 5,
			PAL_XY(271, 40 + 18 * j), kNumColorBlue, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[i], 5,
			PAL_XY(236, 37 + 18 * j++), kNumColorYellow, kNumAlignRight);

		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
			PAL_XY(268, 38 + 18 * j));
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[i], 5,
			PAL_XY(271, 40 + 18 * j), kNumColorBlue, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[i], 5,
			PAL_XY(236, 37 + 18 * j++), kNumColorCyan, kNumAlignRight);

		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
			PAL_XY(268, 38 + 18 * j));
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxSP[i], 5,
			PAL_XY(271, 40 + 18 * j), kNumColorBlue, kNumAlignRight);
		PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwSP[i], 5,
			PAL_XY(236, 37 + 18 * j++), kNumColorGold, kNumAlignRight);

		j = 0;

		PAL_DrawNumber(PAL_GetPlayerAttackStrength(i), 5, PAL_XY(260, 92 + 18 * j++),
			kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerMagicStrength(i), 5, PAL_XY(260, 92 + 18 * j++),
			kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDefense(i), 5, PAL_XY(260, 92 + 18 * j++),
			kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDexterity(i), 5, PAL_XY(260, 92 + 18 * j++),
			kNumColorYellow, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerFleeRate(i), 5, PAL_XY(260, 92 + 18 * j++),
			kNumColorYellow, kNumAlignRight);
		...
		// 绘制物品背景图
		iItemBackgroundPosX = 130;
		iItemBackgroundPosY = 114;
		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ITEMBOX), gpScreen,
			PAL_XY(iItemBackgroundPosX, iItemBackgroundPosY));
		...
		if (i > 0)
		{
			...
			{
				PAL_RLEBlitToSurface(bufImage, gpScreen, PAL_XY(iItemBackgroundPosX + 7, iItemBackgroundPosY + 8));
			}

			//
			// Draw the amount and label of the item
			// 绘制道具名称和库存数量
			lpItemPicText = PAL_GetWord(wItemToUse);
			iItemBackgroundPosX = iItemBackgroundPosX + 32;
			iItemBackgroundPosY = 181;
			PAL_DrawText(lpItemPicText, PAL_XY(iItemBackgroundPosX - PAL_TextWidth(lpItemPicText) / 2, iItemBackgroundPosY), STATUS_COLOR_EQUIPMENT, TRUE, FALSE, FALSE);
			PAL_DrawNumber(i, 3, PAL_XY(iItemBackgroundPosX + PAL_TextWidth(lpItemPicText) / 2, iItemBackgroundPosY + 4), kNumColorGold, kNumAlignRight);
		}
...
--*/
{
	const SDL_Rect      rect = { 20, 8, 300, 198 };
	int                 i, n;
	PAL_LARGE BYTE      bufImage[2048];
	...
	if (__buymenu_firsttime_render)
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 78), 5, FALSE, 6);
	else
		//
		// Draw the amount of this item in the inventory
		//
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 78), 5, FALSE, 0);
	PAL_DrawText(PAL_GetWord(BUYMENU_LABEL_CURRENT), PAL_XY(26, 88), 0, FALSE, FALSE, FALSE);
	PAL_DrawNumber(n, 6, PAL_XY(74, 92), kNumColorYellow, kNumAlignRight);

	if (__buymenu_firsttime_render)
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 118), 5, FALSE, 6);
	else
		//
		// Draw the cash amount
		//
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 118), 5, FALSE, 0);
	PAL_DrawText(PAL_GetWord(CASH_LABEL), PAL_XY(26, 128), 0, FALSE, FALSE, FALSE);
	PAL_DrawNumber(gpGlobals->dwCash, 6, PAL_XY(74, 132), kNumColorGold, kNumAlignRight);

	if (__buymenu_firsttime_render)
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 158), 5, FALSE, 6);
	else
		//
		// Draw the CollectValue amount
		//
		PAL_CreateSingleLineBoxWithShadow(PAL_XY(20, 158), 5, FALSE, 0);
	PAL_DrawText(PAL_GetWord(COLLECTVALUE_LABEL), PAL_XY(26, 168), 0, FALSE, FALSE, FALSE);
	PAL_DrawNumber(gpGlobals->wCollectValue, 6, PAL_XY(74, 172), kNumColorCyan, kNumAlignRight);
	VIDEO_UpdateScreen(&rect);
...
--*/
{
	...
	for (y = 0; y < i; y++)
	{
		...
		PAL_DrawNumber(w, 6, PAL_XY(235, 25 + y * 18), kNumColorGold, kNumAlignRight);
	}
...
/*++
  Purpose:
	...
	当玩家选择另一项时调用的回调函数在销售项目菜单中。

  Parameters:

	[IN]  wCurrentItem - current item on the menu, indicates the object ID of
						 the currently selected item.
	 菜单上的当前项，表示对象ID当前选定的项目。
...
--*/
{
	...
	PAL_DrawNumber(gpGlobals->dwCash, 6, PAL_XY(149, 164), kNumColorGold, kNumAlignRight);
...
/*++
  ...
  Parameters:
	...
	[IN]  wItem - the object ID of the item. 项目的对象ID
...
--*/
{
	...
	while (TRUE)
	{
		...
		PAL_RLEBlitToSurfaceWithShadow(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ITEMBOX), gpScreen,
			PAL_XY(PAL_X(gConfig.ScreenLayout.EquipImageBox) + 5, PAL_Y(gConfig.ScreenLayout.EquipImageBox) + 5), TRUE);

		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ITEMBOX), gpScreen,
			PAL_XY(PAL_X(gConfig.ScreenLayout.EquipImageBox), PAL_Y(gConfig.ScreenLayout.EquipImageBox)));
		...
		{
			// 绘制装备位置名称
			PAL_DrawText(PAL_GetWord(LABEL_EQUEMENT_POSNAME + i), gConfig.ScreenLayout.EquipPosNames[i], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		...
		// 绘制队员属性名称
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_HP), gConfig.ScreenLayout.EquipStatusAttrName[0], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_MP), gConfig.ScreenLayout.EquipStatusAttrName[1], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_SP), gConfig.ScreenLayout.EquipStatusAttrName[2], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_ATTACKPOWER), gConfig.ScreenLayout.EquipStatusAttrName[3], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_MAGICPOWER), gConfig.ScreenLayout.EquipStatusAttrName[4], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_RESISTANCE), gConfig.ScreenLayout.EquipStatusAttrName[5], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_DEXTERITY), gConfig.ScreenLayout.EquipStatusAttrName[6], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);
		PAL_DrawText(PAL_GetWord(STATUS_LABEL_FLEERATE), gConfig.ScreenLayout.EquipStatusAttrName[7], MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);

		//
		// Draw the stats of the currently selected player
		//
		PAL_DrawNumber(PAL_GetPlayerMaxHP(w), 5, gConfig.ScreenLayout.EquipStatusValues[0], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerMaxMP(w), 5, gConfig.ScreenLayout.EquipStatusValues[1], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerMaxSP(w), 5, gConfig.ScreenLayout.EquipStatusValues[2], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerAttackStrength(w), 5, gConfig.ScreenLayout.EquipStatusValues[3], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerMagicStrength(w), 5, gConfig.ScreenLayout.EquipStatusValues[4], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDefense(w), 5, gConfig.ScreenLayout.EquipStatusValues[5], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerDexterity(w), 5, gConfig.ScreenLayout.EquipStatusValues[6], kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_GetPlayerFleeRate(w), 5, gConfig.ScreenLayout.EquipStatusValues[7], kNumColorCyan, kNumAlignRight);
		...
		for (i = 0; i < 10; i++)
		{
			...
			PAL_DrawText(PAL_GetWord(LABEL_RESISTANCE_ALL + i), PAL_XY(285, 2 + i * 20), MENUITEM_COLOR_CONFIRMED, TRUE, FALSE, FALSE);

			if (i < 5)
				PAL_DrawNumber(PAL_GetPlayerElementalResistance(w, i), 3, PAL_XY(302, 26 + i * 20), magicsResistanceColor[i], kNumAlignRight);
		}

		i = 5;

		PAL_DrawNumber(PAL_GetPlayerPoisonResistance(w), 3, PAL_XY(302, 26 + 20 * i++), kNumColorCyan, kNumAlignRight);
		PAL_DrawNumber(PAL_New_GetPlayerSorceryResistance(w), 3, PAL_XY(302, 26 + 20 * i++), kNumColorPink, kNumAlignRight);
		PAL_DrawNumber(PAL_New_GetPlayerPhysicalResistance(w), 3, PAL_XY(302, 26 + 20 * i++), kNumColorGold, kNumAlignRight);
		PAL_DrawNumber(PAL_New_GetPlayerUniqueSkillResistance(w), 3, PAL_XY(302, 26 + 20 * i++), kNumColorBlue, kNumAlignRight);
			...
			if (gpGlobals->g.rgObject[wItem].item.wFlags & (kItemFlagEquipableByPlayerRole_First << w))
			{
				...
				gpGlobals->g.rgObject[wItem].item.wScriptOnEquip =
					PAL_RunTriggerScript(gpGlobals->g.rgObject[wItem].item.wScriptOnEquip,
						gpGlobals->rgParty[iCurrentPlayer].wPlayerRole, FALSE);
			}
		}
	}
}
...
{
	WORD wSavedTimes = 0;
	gpGlobals->bCurrentSaveSlot = (IsAutoSave) ? SAVE_NUMBEROFAUTO : SAVE_NUMBEROFFAST;

	for (BYTE i = 1; i <= SAVE_NUMBEROFAUTO; i++)
		wSavedTimes = (GetSavedTimes(i) > wSavedTimes) ? GetSavedTimes(i) : wSavedTimes;

	PAL_ClearDialog(TRUE);
	PAL_StartDialog(kDialogCenterWindow, 88, 0, FALSE);
	...
	// 非自动存档时，显示"存档完毕"
	if (!IsAutoSave)
		PAL_ShowDialogText(PAL_GetWord(SAVE_COMPLETIONPROMPT));
}
