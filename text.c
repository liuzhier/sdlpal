...
#define   FONT_COLOR_CYAN_ALT       0x2C
...
--*/
{
	...
	g_TextLib.posDialogTitle = PAL_XY(0, 8);
	g_TextLib.posDialogText = PAL_XY(32, 26);
...
#define PROT_OFFSET 10 //consider offset in custom font  考虑自定义字体中的偏移量
		urect.x -= PROT_OFFSET;
	...
	case kDialogUpper:
		...
		if (iNumCharFace > 0)
		{
			...
			if (PAL_MKFReadChunk(buf, PAL_RLEBUFSIZE, iNumCharFace, gpGlobals->f.fpRGM) > 0)
			{
				...
				// HACK To ShengLing
				// 
				if (PAL_RLEGetWidth((LPCBITMAPRLE)buf) >= 112)
					rect.x = 0;
				if (PAL_RLEGetHeight((LPCBITMAPRLE)buf) >= 156)
					rect.y = 0;
		...
		// 根据 是否显示 角色表情 来决定 讲述人名称 和 对话 的横坐标
		//g_TextLib.posDialogTitle = PAL_XY(iNumCharFace > 0 ? 80 : 12, 8);
		//g_TextLib.posDialogText = PAL_XY(iNumCharFace > 0 ? 96 : 44, 26);
		g_TextLib.posDialogTitle = PAL_XY(iNumCharFace > 0 ? 80 : 0, 8);
		g_TextLib.posDialogText = PAL_XY(iNumCharFace > 0 ? 96 : 32, 26);
		break;
	...
	case kDialogLower:
		...
		if (iNumCharFace > 0)
		{
			...
			if (PAL_MKFReadChunk(buf, PAL_RLEBUFSIZE, iNumCharFace, gpGlobals->f.fpRGM) > 0)
			{
				...
				// 
				if (PAL_RLEGetWidth((LPCBITMAPRLE)buf) >= 112)
					rect.x = 320 - PAL_RLEGetWidth((LPCBITMAPRLE)buf);
				if (PAL_RLEGetHeight((LPCBITMAPRLE)buf) >= 156)
					rect.y = 200 - PAL_RLEGetHeight((LPCBITMAPRLE)buf);
		...
		g_TextLib.posDialogTitle = PAL_XY(0, 108);
		g_TextLib.posDialogText = PAL_XY(32, 126);
		break;
	...
	else
	{
		...
		if (g_TextLib.nCurrentDialogLine == 0 &&
			...
			(lpszText[len - 1] == 0xff1a ||
				lpszText[len - 1] == 0x2236 || // Special case for Pal WIN95 Simplified Chinese version         Pal WIN95简体中文版的特殊情况
				lpszText[len - 1] == ':')
			)
		{
			//
...
--*/
{
	...
	// 若特殊符号数量参数为负数，则取 0
	wWordNum = max(wWordNum, 0);


	x = (320 - PAL_TextWidth(lpszText) - wWordNum * 16) / 2;
	y = PAL_Y(g_TextLib.posDialogText) + g_TextLib.nCurrentDialogLine * 18;
	...
	if (g_TextLib.bDialogPosition == kDialogCenterWindow)
	{
...
#endif
		{
			...
			// Create the window box
			// 创建窗口框
			pos = PAL_XY(x, PAL_Y(g_TextLib.posDialogText));
			// Follow behavior of original version
			// 遵循原始版本的行为
			lpBox = PAL_CreateSingleLineBoxWithShadow(pos, (len + 1) / 2, FALSE, g_TextLib.iDialogShadow);
			...
			// Show the text on the screen
			// 在屏幕上显示文本
			TEXT_DisplayText(lpszText, x, PAL_Y(pos) + 10, TRUE);
			VIDEO_UpdateScreen(&rect);
	...
	else
	{
		...
		else
		{
			if (!g_TextLib.fPlayingRNG && g_TextLib.nCurrentDialogLine == 0)
			{
				//
				// Save the screen before we show the first line of dialog
				// 在显示第一行对话框（讲话者名称）之前保存屏幕
				VIDEO_BackupScreen(gpScreen);
			}

			TEXT_DisplayText(lpszText, x, y, FALSE);
