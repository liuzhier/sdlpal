...
--*/
{
   ...
   p = (WORD *)(&gpGlobals->rgEquipmentEffect[kBodyPartfujia]); // HACKHACK
   
   for (i = 0x17; i <= 0x1B; i++)
   {
   p[i * MAX_PLAYER_ROLES + wPlayerRole] = 50;
   }
			
   for (i = 0x05; i <= 0x08; i++)
   {
   PAL_SetPlayerStatus(wPlayerRole, i, 10);
   }
			
   PAL_SetPlayerStatus(wPlayerRole, kStatusfujia1, 10);
}
...
--*/
{
   ...
   j = RandomLong(1, 3);
   p = (WORD *)(&gpGlobals->rgEquipmentEffect[kBodyPartfujia]); // HACKHACK
   iPlayerRole = gpGlobals->rgParty[wPlayerRole].wPlayerRole;
   
   
   if (gpGlobals->fdunjia < 9 && gpGlobals->fdunjia > 1)
       {
	   PAL_BattleShowPlayerDefMagicAnim(wPlayerRole, 0x0239, iPlayerRole);
	   }
	   else
	   {
	if (gpGlobals->fdunjia > 1)
       {
	   PAL_BattleShowPlayerDefMagicAnim(wPlayerRole, 0x012E, iPlayerRole);
	   }
	   }
   ...
   for (i = 0x17; i <= 0x1B; i++)
   {
   p[i * MAX_PLAYER_ROLES + wPlayerRole] = gpGlobals->fdunjia * 10;
   }
   
   for (i = 0x11; i <= 0x15; i++)
   {
   p[i * MAX_PLAYER_ROLES + wPlayerRole] = gpGlobals->fdunjia * 100;
   }
   ...
   if (gpGlobals->fdunjia == 1)
   {
     for (i = 0x05; i <= 0x08; i++)
     {
     PAL_SetPlayerStatus(wPlayerRole, i, gpGlobals->fdunjia);
     }
     PAL_SetPlayerStatus(wPlayerRole, kStatusdunjia, gpGlobals->fdunjia);
     }
   else
   {
	 ...
	 for (i = 0x05; i <= 0x08; i++)
     {
     PAL_SetPlayerStatus(wPlayerRole, i, gpGlobals->fdunjia + 1);
     }
     PAL_SetPlayerStatus(wPlayerRole, kStatusdunjia, gpGlobals->fdunjia + 1); 
	 
	 if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] > 0 && gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] < gpGlobals->fdunjia * 100 && j > 2)
      {
	  gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] = 1;
	  PAL_IncreaseHPMP(wPlayerRole,0, -gpGlobals->fdunjia * 100);
	  }
	  else
	  {
	  PAL_IncreaseHPMP(wPlayerRole,-gpGlobals->fdunjia * 100, -gpGlobals->fdunjia * 100);
	  }
   }
...
#define STAT_LIMIT(t) { if ((t) > 9999) (t) = 9999; }
   ...
   while (wScriptEntry != 0 && !fEnded)
	{
       ...
       pScript->rgwOperand[1], pScript->rgwOperand[2]);
		...
		wEnemyBooty = wEnemyBooty != 0 ? wEnemyBooty : LABEL_NOTHING;
		PAL_DrawNumber(wEnemyBootyNum, 5, pNumPos, kNumColorYellow, kNumAlignRight);
		PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, pUIPos);
